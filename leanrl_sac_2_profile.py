...
if __name__ == "__main__":
    ...
    reward_normalizer = RewardNormalizer(args.gamma, 1.5, args.num_envs).to(device)
    ...
    def update_policy(data):
        ...
        with record_function("q1"):
            qf1_pi, _ = qf1(data["observations"], pi)
        with record_function("q2"):
    ...
    with torch.profiler.profile(schedule=torch.profiler.schedule(skip_first=10, wait=10, warmup=10, active=1)) as prof:
        for global_step in pbar:
            ...
            if global_step < args.learning_starts:
                ...
                with record_function("policy_inference"):
            ...
            with record_function("store_buffer"):
            ...
            with record_function("update_normalizers"):
                update_normalizers(transition)

            # TRY NOT TO MODIFY: CRUCIAL step easy to overlook
            with record_function("data_sampling"):
                obs = next_obs
                data = extend_and_sample(transition)

            with record_function("normalize_batch"):
                data["observations"] = observation_normalizer(data["observations"])
                data["next_observations"] = observation_normalizer(data["next_observations"])
                data["rewards"] = reward_normalizer(data["rewards"])
            ...
            if global_step >= 6000:
                ...
                prof.export_chrome_trace(f"sac.json")
                exit(0)
